...
pub inline fn init(
    ...
    try vk.check(vk.createSwapchainKHR(device, &swapchain_create_info, null, &self.swapchain));
    self.device = device;
    ...
    try vk.check(vk.getSwapchainImagesKHR(self.device, self.swapchain, &self.image_count, null));
...
pub inline fn getImagesBuffered(
    ...
    try vk.check(vk.getSwapchainImagesKHR(self.device, self.swapchain, &image_count, null));
    try list.ensureUnusedCapacity(image_count);
    try vk.check(vk.getSwapchainImagesKHR(self.device, self.swapchain, &image_count, list.unusedCapacitySlice().ptr));
